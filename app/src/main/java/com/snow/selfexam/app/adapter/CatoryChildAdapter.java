...
public class CatoryChildAdapter extends BaseQuickAdapter<CatagoryItemInfo> {
    public CatoryChildAdapter(List<CatagoryItemInfo> data) {
        super(R.layout.item_child_cataory,data);
    }

    //BaseViewHolder:对ViewHolder的封装 不同的页面这个条目 类型与数量==>使用集合封装
    //SparseArray:可以看成HashMap<Integer,Object>
    @Override
    protected void convert(BaseViewHolder helper, CatagoryItemInfo item) {
         //项目编写展示逻辑 setText(参1 id 参2数值)
        helper.setText(R.id.tv_catory_child_item,item.appName);//
